...
The objective is to explore the diversity of [FRB CESAB projects](https://www.fondationbiodiversite.fr/en/about-the-foundation/le-cesab/) through network analysis. The exploration contains three parts:  

1. membership network
2. citation coupling network
...
Among the ```r length(nmem)```  CESAB projects, there were ```r length(unique(cesab$name))``` different researchers. Most of the researcher have been member of a single project (N=```r nproj["1"]```), but ```r nproj["2"]``` researchers collaborated to 2 projects; and ```r sum(nproj[-(1:2)])``` researchers works on 3 projects or more.  

In average, a CESAB project has ```r round(mean(nmem), 1)``` members, with a minimum of ```r  min(nmem)``` for ```r names(which.min(nmem))``` and a maximum of ```r  max(nmem)``` for ```r names(which.max(nmem))```.
...
# ref <- read.csv("data/raw-data/cesab/Zotero_FRB-CESAB.csv")

M <- readRDS(file.path(out_data, "cesab_bibliometrix.rds"))
M$shortjournal <- abbreviate(M$SO, 10)
M$shorttitle <- substr(M$display_name, 1, 30)
...
The network has ```r vcount(net1)``` nodes (one node = one article) and ```r ecount(net1)``` edges (one edge = shared citation between two articles). The connectance is ```r round(edge_density(net1, loops = FALSE),3)``` and the modularity ```r round(modularity(mod1),2)```
